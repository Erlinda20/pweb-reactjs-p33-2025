...
export const getBooks = async (req, res) => {
    try {
        const { page = '1', limit = '10', title, writer, publisher, genre_id, min_price, max_price, min_year, max_year, sort_by = 'created_at', order = 'desc' } = req.query;
        ...
        if (writer) {
            where.writer = { contains: writer, mode: 'insensitive' };
        }
        if (publisher) {
        ...
        if (min_year || max_year) {
            ...
            if (min_year)
                where.publication_year.gte = parseInt(min_year);
            if (max_year)
                where.publication_year.lte = parseInt(max_year);
        }
        // Get total count
